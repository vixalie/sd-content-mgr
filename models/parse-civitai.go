...
func parseCivitaiModelVersionResponse(versionResponse []byte) error {
	var versionInfo *ModelVersion
	err := json.Unmarshal(versionResponse, versionInfo)
	if err != nil {
		return fmt.Errorf("civitai info文件解析失败，%w", err)
	}
	err = persistModelFromVersion(versionInfo)
	if err != nil {
		return fmt.Errorf("无法提取模型信息，%w", err)
	}
	err = persistModelVersion(versionInfo, versionResponse)
	return err
}

// 注意，这个方法生成的Model信息未残缺信息，因为其数据来源是Model Version中携带的反推信息。
func persistModelFromVersion(versionInfo *ModelVersion) error {
	// 首先从数据库中检索一下是否已经存在指定的模型，如果存在则直接返回。
	var model *entities.Model
	db.CacheDB.Where("civitai_model_id = ?", versionInfo.ModelId).First(&entities.Model{}).First(model)
	if model == nil {
		...
		db.CacheDB.Clauses(clause.OnConflict{
...
func persistModelVersion(versionInfo *ModelVersion, original []byte) error {
	var err error
	files, err := persistModelFiles(versionInfo)
	if err != nil {
		return fmt.Errorf("无法提取模型文件信息，%w", err)
	}
	images, err := persistVersionImages(versionInfo)
	if err != nil {
...
func persistModelFiles(versionInfo *ModelVersion) ([]entities.ModelFile, error) {
	...
	db.CacheDB.CreateInBatches(modelFiles, 20)
	return modelFiles, nil
}

// 从civitai的响应中提取出ModelVersion携带的图片信息，但是不会下载图片，所有与实际本地图片有关的字段都将保持为空。
func persistVersionImages(versionInfo *ModelVersion) ([]entities.Image, error) {
	...
	db.CacheDB.CreateInBatches(images, 20)
	return images, nil
}
